		...
		<form class="filter__form form" action="#" method="get" @submit.prevent="submit">
			<fieldset class="form__block">
				...
				<label class="form__label form__label--price">
					<input class="form__input" type="text" placeholder="1" name="min-price" v-model.number="currentPriceFrom">
					<span class="form__value">От</span>
				</label>
				<label class="form__label form__label--price">
					<input class="form__input" type="text" placeholder="100000" name="max-price" v-model.number="currentPriceTo">
			...
			<fieldset class="form__block" v-if="currentCategoryId !=4">
				...
				<ul class="check-list">
					<li class="check-list__item" v-for="(item, index) in category.availableValues" :key="index">
						<label class="check-list__label">
							<input class="check-list__check sr-only" type="checkbox" name="volume" :value="item.value" v-model="currentPropId" :checked="index === 0"/>
			...
			<button class="filter__reset button button--second" type="button" @click.prevent="reset" v-if="currentPriceFrom || currentPriceTo || currentCategoryId || currentColorId.length > 0 || currentPropId.length > 1">
				Сбросить
			</button>
		</form>
	</aside>
	...
	export default {
		...
		computed: {
			...
			category() {
				...
				? this.categoryData.productProps[0] 
				: [];
			},
			colors() {
